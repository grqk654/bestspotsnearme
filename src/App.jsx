...
function EventsWidget({ navigate }) {
  ...
  const key = import.meta.env.VITE_EVENTBRITE_KEY

  useEffect(() => {
    if (!key) { setLoading(false); return }
    const url = `https://www.eventbriteapi.com/v3/events/search/?location.address=Brooklyn%2C%20NY&location.within=10mi&sort_by=date&expand=venue%2Ccategory&token=${key}&page_size=6`
    fetch(url)
      .then(r => r.json())
