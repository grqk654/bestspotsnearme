...
function DiscoverWidget({ navigate }) {
  ...
  const handlePlanDay = () => {
    ...
    const slots = {
      morning:   [{ label: 'Start here',   cats: ['coffee', 'bakeries'] }],
      afternoon: [{ label: 'Grab a bite',  cats: ['burgers', 'ramen'] }],
      evening:   [{ label: 'Wind down',    cats: ['bars', 'brunch'] }],
      fullday:   [
        { label: 'Start here',  cats: ['coffee', 'bakeries'] },
        { label: 'Grab a bite', cats: ['burgers', 'ramen'] },
        { label: 'Wind down',   cats: ['bars', 'brunch'] },
      ],
    }
    const plan = (slots[dayTime] || slots.fullday).map((slot, i) => ({
      time: ['Morning', 'Afternoon', 'Evening'][i] || slot.label,
      label: slot.label,
      listing: getFrom(slot.cats, dayHood),
    }))
    setAnimKey(k => k + 1)
    setDayPlan(plan)
  }
  ...
  const pillStyle = (key) => {
    const isActive = selCat === key
    const catColor = CAT[key]
  ...
  const timeBtnStyle = (key) => ({
    flex: 1, padding: '9px 8px', borderRadius: 8,
    border: `1.5px solid ${dayTime === key ? T.navy : T.border}`,
    background: dayTime === key ? T.navy : T.bg,
    color: dayTime === key ? T.navyText : T.textMid,
  ...
  const ResultCard = ({ listing, stopMeta }) => {
    ...
    return (
      <div key={listing.id} style={{ border: `1.5px solid ${listing.featured ? T.coral : T.border}`, borderRadius: 12, padding: 16, background: listing.featured ? '#FFFAF9' : T.bg, marginBottom: 10 }}>
        {stopMeta && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12, paddingBottom: 10, borderBottom: `0.5px solid ${T.border}` }}>
            <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: T.textMuted }}>{stopMeta.time}</span>
            <div style={{ width: 3, height: 3, borderRadius: '50%', background: T.border }} />
            <span style={{ fontSize: 11, color: T.textMuted }}>{stopMeta.label}</span>
          </div>
        )}
        {listing.featured && (
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 600, padding: '3px 8px', borderRadius: 4, background: cat.bg, color: cat.text, border: `1px solid ${cat.border}`, whiteSpace: 'nowrap', flexShrink: 0, textTransform: 'capitalize' }}>
            {cat.icon} {listing.category}
          </span>
        </div>
        <div style={{ fontSize: 12, color: T.textMid, marginBottom: 10, display: 'flex', alignItems: 'center', gap: 5 }}>
          <span style={{ color: '#BA7517' }}>{'★'.repeat(Math.round(listing.rating))}</span>
          <span>{listing.rating}</span>
          <span style={{ opacity: 0.4 }}>·</span>
          <span>{listing.neighborhood}, Brooklyn</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <a href={listing.mapsUrl} target="_blank" rel="noopener noreferrer"
            style={{ padding: '8px 16px', background: T.coral, color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 500, textDecoration: 'none', fontFamily: "'DM Sans', sans-serif" }}>
            Get directions
          </a>
          <button onClick={() => navigate(`listing/${listing.id}`)}
            style={{ padding: '8px 14px', background: T.bg, color: T.text, border: `1.5px solid ${T.border}`, borderRadius: 8, fontSize: 13, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
            View spot →
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ border: `1.5px solid ${T.border}`, borderRadius: 14, padding: 22, background: T.bgSoft, margin: '0 0 32px', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: 'flex', border: `1.5px solid ${T.border}`, borderRadius: 10, overflow: 'hidden', marginBottom: 20 }}>
        {['quick', 'day'].map((t, i) => (
          <button key={t} onClick={() => setTab(t)}
            style={{ flex: 1, padding: '9px 16px', fontSize: 13, fontWeight: 500, background: tab === t ? T.navy : T.bg, color: tab === t ? T.navyText : T.textMid, border: 'none', borderRight: i === 0 ? `1px solid ${T.border}` : 'none', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
            {t === 'quick' ? 'Quick pick' : 'Plan my day'}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
            {Object.keys(CAT_LABELS).map(key => (
              <button key={key} onClick={() => setSelCat(key)} style={pillStyle(key)}>
                {key !== 'any' && CAT[key] ? `${CAT[key].icon} ` : ''}{CAT_LABELS[key]}
              </button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 10, marginBottom: 6, alignItems: 'center' }}>
            <select value={quickHood} onChange={e => setQuickHood(e.target.value)} style={selectStyle}>
              {hoods.map(h => <option key={h} value={h}>{h === 'any' ? 'Any neighborhood' : h}</option>)}
            </select>
            <button onClick={handleQuickPick}
              style={{ padding: '10px 20px', background: T.coral, color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: "'DM Sans', sans-serif" }}>
              Find a spot
            </button>
          </div>
          {quickResult && quickResult !== 'empty' && (
            <div key={animKey} style={{ marginTop: 18 }}>
              <ResultCard listing={quickResult} />
              <button onClick={handleQuickPick}
                style={{ width: '100%', padding: '9px', background: 'transparent', border: `0.5px solid ${T.border}`, borderRadius: 8, fontSize: 13, color: T.textMid, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                Try another spot →
              </button>
            </div>
          )}
          {quickResult === 'empty' && (
            <div style={{ marginTop: 16, padding: 20, border: `0.5px dashed ${T.border}`, borderRadius: 10, textAlign: 'center', fontSize: 13, color: T.textMuted }}>
              No spots found for that combo — try a different neighborhood or category.
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'day' && (
        <div>
          <div style={s.sectionLabel}>When are you going?</div>
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            {[['fullday', 'Full day'], ['morning', 'Morning'], ['afternoon', 'Afternoon'], ['evening', 'Evening']].map(([key, label]) => (
              <button key={key} onClick={() => setDayTime(key)} style={timeBtnStyle(key)}>{label}</button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 10, marginBottom: 6, alignItems: 'center' }}>
            <select value={dayHood} onChange={e => setDayHood(e.target.value)} style={selectStyle}>
              {hoods.map(h => <option key={h} value={h}>{h === 'any' ? 'Any neighborhood' : h}</option>)}
            </select>
            <button onClick={handlePlanDay}
              style={{ padding: '10px 20px', background: T.coral, color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: "'DM Sans', sans-serif" }}>
              Plan it
            </button>
          </div>
          {dayPlan && (
            <div key={animKey} style={{ marginTop: 18 }}>
              {dayPlan.map((stop, i) => (
                <ResultCard key={i} listing={stop.listing} stopMeta={{ time: stop.time, label: stop.label }} />
              ))}
              <button onClick={handlePlanDay}
                style={{ width: '100%', padding: '9px', background: 'transparent', border: `0.5px solid ${T.border}`, borderRadius: 8, fontSize: 13, color: T.textMid, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                Remix this day →
